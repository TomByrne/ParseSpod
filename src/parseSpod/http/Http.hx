...
class Http<T> extends Stream<T>
{
	...
	public function send() : Http<T>
	{
		try{
			if (_innerhttp == null){
				_innerhttp = new BaseHaxeHttp(url);
			...
			if (_filePath != null){
				...
				#if sys
					_filePath = FileSystemTools.cleanupNativePath(_filePath);
					var filename:String = FileSystemTools.getName(_filePath);
